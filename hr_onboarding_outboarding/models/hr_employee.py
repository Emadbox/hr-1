...
class hr_employee_with_onboarding_outboarding_projects(models.Model):
    ...
    def action_create_onboarding_project(self):
        for employee in self:
            ...
            if len(onboarding_project_template) == 0:
                raise ValidationError('No OutBoarding project defined! - There is no Out-Boarding project defined in your system.')
            ...
            project_id = self.env['project.project'].create({
                ...
                'on_out_boarding_employee_id': self.id,
            })
            # Add tasks
